...
Boundary points are allowed, including obstacle edges. The low-level algorithm uses a point: author inset floors or expanded obstacles when physical clearance matters. `PhaserAdventureNavigation` expands registered entity footprints by the mover's footprint automatically. The algorithm targets room-sized adventure scenes with dozens of polygon vertices; it is not a crowd-navigation system. `pointInPolygon`, `isWalkable`, `isSegmentWalkable`, `closestPointOnPolygon` and `distance` are available for previews and custom tools.
...
An assignment can set `reverse: true` to play the same clip backwards, including its per-frame delays and transforms, without modifying the shared asset. The binding's `animationDurationMs` reports one full cycle of the selected clip, including authored holds. The forest guard uses the cycle duration to finish his idle and drinking clips before advancing his patrol; direction changes are immediate.
...
bindAdventureSpriteInteraction(sprite, {
  enabled: () => !editorOpen && !dialogOpen,
  onInteract: () => approachAndInteract(entity),
  onLook: () => showDescription(entity),
});
bindAdventureInput(scene, {
  enabled: () => !editorOpen && !dialogOpen,
  resolve(pointer) {
    const point = pointer.positionToCamera(scene.cameras.main);
    const target = findHotspot(point);
    return {
      onInteract: () => target ? approachAndInteract(target) : walkTo(point),
      onLook: () => { if (target) showDescription(target); },
    };
  },
});
```
