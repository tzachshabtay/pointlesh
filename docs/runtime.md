...
An assignment can set `reverse: true` to play the same clip backwards, including its per-frame delays and transforms, without modifying the shared asset. The binding's `animationDurationMs` reports one full cycle of the selected clip, including authored holds. The forest guard uses these to finish each turn/drink cycle before advancing his patrol and to reuse turn clips on the return journey.
