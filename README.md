...
The current integration uses Scene Designer `^0.2.0`, AI Assets `^0.8.4`, Dialog Designer `^0.1.1` and Phaser `^4.2.0`. Scene Designer 0.2's published peer metadata still specifies an older AI Assets 0.7 range. This checkout deliberately resolves one compatible AI Assets 0.8.4 family and exercises it through the build and tests. Downstream npm applications using these package artifacts need the same application-level overrides until upstream peer ranges are updated:

```json
{
  "overrides": {
    "@scene-designer/core": { "@ai-game-assets/core": "^0.8.4" },
    "@scene-designer/designer": { "@ai-game-assets/core": "^0.8.4" },
    "@scene-designer/phaser": {
      "@ai-game-assets/core": "^0.8.4",
      "@ai-game-assets/phaser": "^0.8.4"
    }
  }
}
```
