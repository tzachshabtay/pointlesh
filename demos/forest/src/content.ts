...
export const scenes = addForestPoints(updateForestInteractions(specializeForestEntities(defineSceneManifest({ schemaVersion: 2, prefabs: base, scenes: Object.fromEntries(roomIds.map(roomId => {
  ...
  const areas = [
    createPointleshArea({ id: `${roomId}.floor::area`, entityId: `${roomId}.floor`, name: 'Walkable ground & perspective', walkable: true, scaleEnabled: true, zoomEnabled: true, vertices: roomFloorVertices(roomId), closed: true, minScale: 0.75, maxScale: 1.22, minZoom: 1.035, maxZoom: 1 }),
    createPointleshArea({ id: `${roomId}.foreground::area`, entityId: `${roomId}.foreground`, name: 'Foreground occlusion', walkBehindEnabled: true, vertices: roomForegroundVertices(roomId), closed: true, baseline: 505 }),
    ...targets[roomId].filter(target => !(roomCharacters[roomId] ?? []).some(npc => npc.actorName !== 'king' && npc.actorName === target.id) && !(roomPickups[roomId] ?? []).some(pickup => pickup.pickupId === target.id)).map(target => createPointleshArea({ id: `${target.id}::area`, entityId: target.id, kind: 'hotspot', name: target.name, properties: { label: target.name, exit: target.exit ?? '', description: target.description }, behaviors: ['forest.interact'], vertices: rectangle(target.x - 34, target.y - 35, 68, 64), closed: true, approachX: target.walkX ?? target.x, approachY: target.walkY ?? Math.max(403, Math.min(494, target.y + 25)) }))
  ];
