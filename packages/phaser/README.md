...
For cutscenes and animation previews, `view.renderPose({ position, activity, facing, scale? }, elapsedMs, { loop? })` samples a clip at absolute time using the same assignments, frame holds, transforms, previews, and scaled variants. It loops by default; `{ loop: false }` holds the final frame after the clip ends. Omitting `scale` evaluates the configured areas at the pose's position, with the same authored base size and scale as gameplay. It leaves the gameplay controller and its movement path untouched. Use a separate sprite/binding with `autoUpdate: false` for a cinematic cast, and supply the cutscene checkpoint's elapsed time to restore the exact pose. Call `sync()` or `update()` to resume displaying the controller.
